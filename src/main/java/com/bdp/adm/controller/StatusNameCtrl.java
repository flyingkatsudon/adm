...
public class StatusNameCtrl {
	@Autowired
	private StatusNameBiz statusNameBiz;
	...
	// Select
	@RequestMapping(value = "/selectAllStatusName", method = RequestMethod.GET, produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String selectAllStatusName(String action) throws JsonProcessingException {
		
		return statusNameBiz.selectAllStatusName(action);
	}
	
	// Edit
	@RequestMapping(value = "/editStatusName", method = RequestMethod.POST, produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String editStatusName(@RequestBody String responseData, HttpServletRequest request) throws URISyntaxException, JsonMappingException, IOException {
		try {
			...
			if(action.equals("create")) {
				...
				res = statusNameBiz.insertStatusName(data, currentUser);
				System.out.println("CREATE RESULT: " + res);
			}
			else if (action.equals("edit")) {
				System.out.println("### EDIT ###");
				res = statusNameBiz.updateStatusName(data, currentUser);
				System.out.println("EDIT RESULT: " + res);
			}
			else if (action.equals("remove")) {
				System.out.println("### REMOVE ###");
				res = statusNameBiz.deleteStatusName(data, currentUser);
			...
			return this.selectAllStatusName(action);
