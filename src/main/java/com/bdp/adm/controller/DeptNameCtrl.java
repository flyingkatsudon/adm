...
public class DeptNameCtrl {
	@Autowired
	private DeptNameBiz deptNameBiz;
	
	@RequestMapping(value = "/deptNameList", method = RequestMethod.GET)
	...
	// Select
	@RequestMapping(value = "/selectAllDeptName", method = RequestMethod.GET, produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String selectAllDeptName(String action) throws JsonProcessingException {
		
		return deptNameBiz.selectAllDeptName(action);
	}
	
	// Edit
	@RequestMapping(value = "/editDeptName", method = RequestMethod.POST, produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String editDeptName(@RequestBody String responseData, HttpServletRequest request) throws URISyntaxException, JsonMappingException, IOException {
		try {
			...
			if(action.equals("create")) {
				...
				res = deptNameBiz.insertDeptName(data, currentUser);
				System.out.println("CREATE RESULT: " + res);
			}
			else if (action.equals("edit")) {
				System.out.println("### EDIT ###");
				res = deptNameBiz.updateDeptName(data, currentUser);
				System.out.println("EDIT RESULT: " + res);
			}
			else if (action.equals("remove")) {
				System.out.println("### REMOVE ###");
				res = deptNameBiz.deleteDeptName(data, currentUser);
			...
			return this.selectAllDeptName(action);
