...
public class RoleCtrl {
	...
	// Select
	@RequestMapping(value = "/selectAllRole", method = RequestMethod.GET, produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String selectAllRole(String action) throws JsonProcessingException {
		
		return roleBiz.selectAllRole(action);
	}
	
	// Edit
	@RequestMapping(value = "/editRole", method = RequestMethod.POST, produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String editRole(@RequestBody String responseData, HttpServletRequest request) throws URISyntaxException, JsonMappingException, IOException {
		...
		try {
			...
			if(action.equals("create")) {
				...
				res = roleBiz.insertRole(data, currentUser);
				System.out.println("CREATE RESULT: " + res);
			}
			else if (action.equals("edit")) {
				System.out.println("### EDIT ###");
				res = roleBiz.updateRole(data, currentUser);
				System.out.println("EDIT RESULT: " + res);
			}
			else if (action.equals("remove")) {
				System.out.println("### REMOVE ###");
				res = roleBiz.deleteRole(data, currentUser);
			...
			return this.selectAllRole(action);
