$(document).ready(function() {
    ...
    editor = new $.fn.dataTable.Editor( {
    	...
    	ajax: function ( method, url, data, success, error ) {
    		...
    		$.ajax({
				url : myUrl + "adm/crawl/editCrawlKeywords",
				type: "POST",
				data: data,
				dataType: "json",
	...
	onRequestAllCrawlKeywords();
    
    function onRequestAllCrawlKeywords() {
    	$.ajax({
			url : myUrl + "adm/crawl/selectAllCrawlKeywords",
	    	type: "GET",
	    	dataType: "json",
    ...
    function onDrawTable(data) {
        ...
        var curTable = $('#crawlKeywords').DataTable({
            dom: "Bfrtip",
            data :data,
            columnDefs: [
    	        {
        ...
        editor.on( 'onOpen', function () {
        	...
        	$( 'input', this.node( 'cnt' ) ).attr( 'maxLength', 100 ).attr('hname','카운트').attr('types','NUMB').attr('CHKREQ', 'false');
        	$( 'input', this.node( 'registrantInfo' ) ).attr( 'maxLength', 20 ).attr('hname','등록자 정보').attr('types','').attr('CHKREQ', 'true');
