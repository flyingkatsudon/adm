$(document).ready(function() {
	...
	function onSetMeta(data) {
		...
		editor = new $.fn.dataTable.Editor({
	    	...
	    	ajax: function ( method, url, data, success, error ) {
	    		...
	    		$.ajax({
					url : myUrl + "adm/link/editScreenLink",
					type: "POST",
					data: data,
					dataType: "json",
    ...
    function onRequestAllScreenLink() {
    	$.ajax({
			url : myUrl + "adm/link/selectAllScreenLink",
	    	type: "GET",
	    	dataType: "json",
    ...
    function onDrawTable(data) {
    	...
    	var curTable = $('#screenLink').DataTable( {
    	    dom: "Bfrtip",
    	    data :data,
    	    columnDefs: [
    	        {
        ...
        editor.on( 'preSubmit', function ( e, data, action ) {
        	...
        	for (var el in d) {
        		for (var col in d[el]) {
        			if( col=='loadDate' || col=='loadTime' || col=='loadName' || col=='updateDate' || col=='updateTime' || col=='updateName' || col=='linkIdx' ) break;
        			var curCol    = d[el][col];
