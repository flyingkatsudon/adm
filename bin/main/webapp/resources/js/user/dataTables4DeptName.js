$(document).ready(function() {
	...
	onRequestAllDeptName();
    
    function onRequestAllDeptName() {
    	$.ajax({
			url : myUrl + "adm/dn/selectAllDeptName",
	    	type: "GET",
	    	dataType: "json",
    ...
    function onDrawTable(data) {
        ...
        var curTable = $('#deptName').DataTable({
            dom: "Bfrtip",
            data :data,
            columnDefs: [
    	        {
        ...
        editor.on('onInitCreate', function () {
        	editor.enable('userDeptCd');
        	editor.hide('delCd');
    	});
    	
        editor.on('onInitEdit', function () {
        	editor.disable('userDeptCd');
        	editor.show('delCd');
    	});
        
        editor.on( 'onOpen', function () {
        	$( 'input', this.node( 'userDeptCd' ) ).attr( 'maxLength', 4 ).attr('hname','부서코드').attr('types','').attr('CHKREQ', 'true');
        	$( 'input', this.node( 'userDeptName' ) ).attr( 'maxLength', 30 ).attr('hname','부서명').attr('types','').attr('CHKREQ', 'false');
        });
        ...
        editor.on( 'preSubmit', function ( e, data, action ) {
        	...
        	for (var el in d) {
        		for (var col in d[el]) {
        			if( col=='loadDate' || col=='loadTime' || col=='loadName' || col=='updateDate' || col=='updateTime' || col=='updateName' || col=='deptIdx' ) break;
        			var curCol    = d[el][col];
